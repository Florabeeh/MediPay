...
export default function MediPay() {
  ...
  const [authEmail, setAuthEmail] = useState(""); const [authPw, setAuthPw] = useState(""); const [authErr, setAuthErr] = useState(""); const [showPw, setShowPw] = useState(false); const [verifyEmail, setVerifyEmail] = useState("");
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const tx = await sendPayment(API_KEY, walletId, usdc);
    ...
    <Shell {...shellProps}>
      ...
      <div style={s.pg}>
        ...
        <div style={s.rcpCard}>
          ...
          <p style={{ fontSize: 10, color: palette.muted, textAlign: "center", wordBreak: "break-all", marginTop: 4 }}>
            Tx: {receipt?.arcTxHash
              ? <a href={"https://testnet.arcscan.app/tx/" + receipt.arcTxHash} target="_blank" rel="noreferrer" style={{ color:"#20b2aa", textDecoration:"none" }}>{receipt.arcTxHash}</a>
              : receipt?.id}
          </p>
          {!receipt?.arcTxHash && receipt?.walletAddr && <p style={{ textAlign:"center", marginTop:4 }}><a href={"https://testnet.arcscan.app/address/" + receipt.walletAddr + "?tab=token_transfers"} target="_blank" rel="noreferrer" style={{ fontSize:10, color:"#20b2aa", textDecoration:"none", fontWeight:600 }}>View all transfers on ARC Explorer</a></p>}
        </div>
            ...
            <div key={r.id} style={{ ...s.histCard, ...(r.status === "pending" ? { borderLeft: "4px solid #f5c85b" } : { borderLeft: "4px solid #3fb7a3" }) }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {r.arcTxHash ? <a href={"https://testnet.arcscan.app/tx/" + r.arcTxHash} target="_blank" rel="noreferrer" style={{ fontSize: 10, color: palette.brandDeep, textDecoration: "none", marginTop: 2, display: "block" }}>View on ARC Explorer</a> : r.walletAddr ? <a href={"https://testnet.arcscan.app/address/" + r.walletAddr + "?tab=token_transfers"} target="_blank" rel="noreferrer" style={{ fontSize: 10, color: palette.brandDeep, textDecoration: "none", marginTop: 2, display: "block" }}>View transfers on ARC Explorer</a> : null}
  ...
  <div style={{
    ...
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <button onClick={() => setShowPw(!showPw)} type="button" style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", fontSize:18, color:"#9ca3af", lineHeight:1 }}>{showPw ? (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>) : (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>)}</button>
        </div>
